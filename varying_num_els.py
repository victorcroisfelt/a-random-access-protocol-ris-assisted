...
num_configs_range = np.array([4])#np.array([1, 2, 4, 8])
...
throughput = np.zeros((num_configs_range.size, num_els_range.size, num_setups))
...
num_active_ue_range = np.random.binomial(num_ue, prob_access, size=num_setups).astype(int)
...
for ss, num_active_ue in enumerate(num_active_ue_range):
    ...
    #print(f"\tsetup: {ss}/{num_active_ue_range.size-1}")
    ...
    for cc, num_configs in enumerate(num_configs_range):
        ...
        for nn, num_els in enumerate(num_els_range):
            ...
            # Place RIS
            box.place_ris(num_configs=num_configs, num_els_v=num_els, num_els_h=num_els)
            ...
            ##################################################
            throughput[cc, nn, ss] = throughput_evaluation_with_power(ue_choices, gamma_ul, gamma_th)
...
for cc, num_configs in enumerate(num_configs_range):
    ...
    if num_configs == 1:
        ax.plot(num_els_range, np.nanmean(normalized_throughput[cc], axis=-1), label='Slotted-ALOHA')

    else:
        ax.plot(num_els_range, np.nanmean(normalized_throughput[cc], axis=-1), label='RIS-assisted: $S =' + str(num_configs) + '$')
...
np.savez('data/num_els_' + choose_config + '_Sc' + str(num_chosen_configs) + '.npz',
