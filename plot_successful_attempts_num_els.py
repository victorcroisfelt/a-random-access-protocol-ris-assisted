...
# Load data
aloha_data = np.load('data/aloha_varyingN.npz')
strongest_data = np.load('data/ris_strongest_varyingN.npz')
rand_data = np.load('data/ris_rand_varyingN.npz')
...
num_els_range = strongest_data['num_els_range']
...
for mm in range(3):
    ...
    # Legend use
    ax.plot(num_els_range, np.nanmean(num_successful_attempts[mm, 0], axis=-1), linewidth=2.0, linestyle=styles[mm],
            color=colors[mm], label=methods[mm])

    # Go through all number of configurations
    for cc, num_configs in enumerate(num_configs_range):

        ax.plot(num_els_range, np.nanmean(num_successful_attempts[mm, cc], axis=-1), linewidth=2.0, marker=markers[cc],
...
for cc, num_configs in enumerate(num_configs_range):
    ...
    # Legend use
    ax.plot(num_els_range, np.nanmean(num_successful_attempts[0, cc], axis=-1), linewidth=None, linestyle=None,
            marker=markers[cc], color='black', label='$S =' + str(num_configs) + '$')

# Set axis
ax.set_xlabel(r'number of elements along axes, $N_x$, $N_z$')
...
plt.savefig('figs/num_successful_attempts_varyingN.pdf', dpi=300)
